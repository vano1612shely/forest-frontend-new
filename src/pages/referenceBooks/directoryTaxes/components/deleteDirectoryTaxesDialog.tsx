...
import {
	deleteDirectoryTaxes,
	directoryTaxesOptions
} from '@/api/referenceBooks'

export const DeleteDirectoryTaxesDialog = ({ id }: { id: string }) => {
	const queryClient = useQueryClient()
	const { mutate } = useMutation({
		mutationKey: ['deleteDirectoryTaxes'],
		mutationFn: () => deleteDirectoryTaxes(id),
		onSuccess: () => {
			queryClient.refetchQueries(directoryTaxesOptions())
			toast.success('Форма оподаткування успішно видалена')
		},
		onError: error => {
