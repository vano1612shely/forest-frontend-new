...
import {
	deleteDirectoryOwnership,
	directoryOwnershipOptions
} from '@/api/referenceBooks'

export const DeleteDirectoryOwnershipDialog = ({ id }: { id: string }) => {
	const queryClient = useQueryClient()
	const { mutate } = useMutation({
		mutationKey: ['deleteDirectoryTaxes'],
		mutationFn: () => deleteDirectoryOwnership(id),
		onSuccess: () => {
			queryClient.refetchQueries(directoryOwnershipOptions())
			toast.success('Форма власності успішно видалена')
		},
		onError: error => {
